...
- 상태 선택은 드롭다운보다 상태 버튼 그룹을 우선하되, 큰 스티커처럼 보이지 않게 작고 밀도 있게 둔다.
- 상태 버튼 텍스트는 줄바꿈되지 않아야 한다.
- 상태 구분은 MVP에서는 고정 옵션으로 두지만, 운영 단계에서는 관리자 모드에서 추가/삭제/정렬 가능한 마스터 데이터로 전환한다.
...
- 좁은 모바일에서는 `이슈 현황 / 미종료 관리 / 최근 변동` 요약 카드를 긴 세로 카드가 아니라 한 줄 compact 요약으로 줄인다.
