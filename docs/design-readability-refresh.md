...
- `기록 유형`은 상태가 아니라 `회의 / 시험 / 분석 / 보고 / 조치 / 승인 / 고객대응 / 기타`로 따로 고른다.
- 상태, 세부 단계, 기록 유형 버튼은 큰 스티커처럼 보이지 않게 작고 밀도 있게 둔다.
- 버튼 텍스트는 줄바꿈되지 않아야 한다.
- 상태/세부 단계/기록 유형 구분은 MVP에서는 고정 옵션으로 두지만, 운영 단계에서는 관리자 모드에서 추가/삭제/정렬 가능한 마스터 데이터로 전환한다.
