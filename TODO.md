...
- [ ] Drawer QA: 모바일에서 `이력 추가` drawer의 `대분류 > 하위 주제 > 이슈 > 세부 항목` 선택, 새 항목 입력, 상태/세부 단계/기록 유형 버튼, 여러 첨부 URL 입력, 하단 저장 버튼이 겹치지 않는지 확인한다.
...
- [ ] 관리자 모드에서 업무 라벨과 기록 유형 옵션을 추가/삭제/정렬하는 마스터 관리 화면을 설계한다.
